...
* __1.可针对任务模式(如IO密集型任务和CPU密集任务)及重要性对线程的CPU核数绑定，调整操作系统对用户任意任务的调度策略与优先级__
...
```c++
std::cout << "thee result1 is :" << res.get(); << std::endl;
std::cout << "thee result2 is :" << res2.get(); << std::endl;
```
------
### 异步获取任务返回值
* 后续会加入日志系统
